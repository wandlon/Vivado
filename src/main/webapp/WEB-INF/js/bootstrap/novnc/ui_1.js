...
(function () {
    ...
    UI = {
        ...
        load: function (callback, vmId) {
            WebUtil.initSettings(UI.start(callback, vmId), callback);
        },

        // Render default UI and initialize settings menu
        start: function(callback, vmId) {
            ...
            UI.setViewClip(null, vmId);
            UI.setBarPosition(vmId);

            Util.addEvent(window, 'resize', function () {
                UI.onresize(null, vmId);
                UI.setViewClip(null, vmId);
        ...
        initRFB: function (vmId) {
            try {
                UI.rfb = new RFB({'target': $D('noVNC_canvas_' + vmId),
                ...
                UI.updateState(null, 'fatal', null, 'Unable to create RFB client -- ' + exc, vmId);
                return false;
            }
        },
        ...
        onresize: function (callback, vmId) {
            if (!UI.rfb) return;

            var size = UI.getCanvasLimit(vmId);
        ...
        getCanvasLimit: function (vmId) {
            var container = $D('noVNC_container_' + vmId);
            //alert(container);
        ...
        updateState: function(rfb, state, oldstate, msg, vmId) {
            UI.rfb_state = state;
            var klass;
            switch (state) {
                case 'failed':
                ...
                case 'normal':
                    ...
                    break;
                case 'disconnected':
                    $D('noVNC_logo_' + vmId).style.display = "block";
                    $D('noVNC_container_' + vmId).style.display = "none";
                    /* falls through */
                case 'loaded':
                    klass = "noVNC_status_normal";
                    break;
        ...
        updateVisualState: function(vmId) {
            var connected = UI.rfb && UI.rfb_state === 'normal';
        ...
        connect: function(vmId, vncHostIp, vncPort) {
        	serverId = vmId;
        	UI.load(null, vmId);
            UI.closeSettingsMenu();
            UI.toggleConnectPanel();
            ...
            //Close dialog.
            setTimeout(UI.setBarPosition(vmId), 100);
            $D('noVNC_logo_' + vmId).style.display = "none";
            $D('noVNC_container_' + vmId).style.display = "inline";
        },
        ...
        setViewClip: function(clip, vmId) {
            var display;
            if (UI.rfb) {
            ...
            if (UI.getSetting('clip')) {
                ...
                if (size) {
                    ...
                    $D('noVNC_container_' + vmId).style.overflow = "hidden";

                    // The x position marks the left margin of the canvas,
                    // remove the margin from both sides to keep it centered
                    var new_w = size.w - (2 * Util.getPosition($D('noVNC_canvas_' + vmId)).x);

                    $D('noVNC_container_' + vmId).style.overflow = "visible";
        ...
        setBarPosition: function(vmId) {
            $D('noVNC-control-bar').style.top = (window.pageYOffset) + 'px';
            $D('noVNC_mobile_buttons').style.left = (window.pageXOffset) + 'px';
            alert(vmId);
            var vncwidth = $D('noVNC_screen_' + vmId).style.offsetWidth;
